...
“章境”是一款面向长篇小说的原生 Android 应用。它把 TXT 小说转换成可阅读、可对话、可检索、可视化并可配音的故事世界。当前版本为 `0.6.0 (12)`。
...
- 聊天/原文双模式阅读，使用正文字符偏移稳定恢复位置；目录、书签、高亮、批注和书内全文搜索集中在阅读导航中。
- 纸白、羊皮纸和夜间阅读主题，支持全局默认与单书字号、行距、边距、字体覆盖。
...
- Media3 后台听书支持锁屏与蓝牙控制、章节进度、倍速、睡眠定时、段落跟随及全局迷你播放器；缺少下一章音频时停止并提示，不会自动生成。
- “任务”页面显示正在执行及近七天的分析、配音与全文索引记录。
...
环境要求：JDK 17、Android SDK 36；应用 targetSdk 为 35、minSdk 为 26。
